...
export class BackgroundService {
  ...
  fetch<Api extends keyof AllApiTypes, Method extends keyof AllApiTypes[Api]>
    (api: Api, method: Method): (args: Parameters<AllApiTypes[Api][Method]>) => Observable<ReturnType<AllApiTypes[Api][Method]>>
  {
    return (args) => {
      const id = this.makeId();
      ...
      return this.listenEvents<ReturnType<AllApiTypes[Api][Method]>>(id).pipe(
        take(1),
        switchMap(event => {
          if (event!.status === 'error') {
            return throwError(() => event!.data);
  ...
  reduce<Api extends keyof Reducers, Method extends keyof Reducers[Api]>
    (api: Api, method: Method): (data: Parameters<Reducers[Api][Method]>[0]) => Observable<ReturnType<Reducers[Api][Method]>>
  {
    return (data) => {
      const id = this.makeId();
      ...
      return this.listenEvents<ReturnType<Reducers[Api][Method]>>(id).pipe(
        take(1),
        switchMap(event => {
          if (event!.status === 'error') {
            return throwError(() => event!.data);
